...
var gameApi = __webpack_require__(10);
...
var logicCheck = function logicCheck(event) {
  var i = event.target.id.slice(0, 1);
...
var onAllGames = function onAllGames(event) {
  ...
  gameApi.allGames().then(gameUi.indexSuccess).catch(gameUi.apiFail);
};
//
...
var onNewGame = function onNewGame(event) {
  ...
  gameApi.newGame(data).then(gameUi.gameStart).catch(gameUi.apiFail);
};
// onPatchGame updates the api with the gameBoard's info
var onPatchGame = function onPatchGame(event) {
  event.preventDefault();
  var i = event.target.id.slice(0, 1);
  var data = {
    "game": {
  ...
  gameApi.patchGame(data).then(gameUi.apiUpdate).catch(gameUi.apiFail);
};
//
...
// event listeners
var gameplayHandlers = function gameplayHandlers() {
...
//
module.exports = {
  gameplayHandlers: gameplayHandlers,
  onEndGame: onEndGame
};
...
var current = {
  ...
  changePlayer: function changePlayer() {
    if (this.player == '🐱') {
      this.player = '🐶';
      //this.nextPlayer = '🐱'
    } else if (this.player == '🐶') this.player = '🐱';
    //this.nextPlayer = '🐶'
  },
  over: false,
...
//
var updateBoard = function updateBoard(i, player) {
...
var winConditions = function winConditions() {
  if (store.game.cells[0] === '🐱' && store.game.cells[1] === '🐱' && store.game.cells[2] === '🐱' || store.game.cells[0] === '🐶' && store.game.cells[1] === '🐶' && store.game.cells[2] === '🐶') {
    ...
    current.winner = current.player + ' wins';
    current.endGame();
  } else if (store.game.cells[0] !== undefined && store.game.cells[1] !== undefined && store.game.cells[2] !== undefined && store.game.cells[3] !== undefined && store.game.cells[4] !== undefined && store.game.cells[5] !== undefined && store.game.cells[6] !== undefined && store.game.cells[7] !== undefined && store.game.cells[8] !== undefined && store.game.over === false) {
...
// get one past game
var oneGame = function oneGame(id) {
  return $.ajax({
    method: 'GET',
...
// update game
var patchGame = function patchGame(data) {
  return $.ajax({
    method: 'PATCH',
...
var gameLogic = __webpack_require__(5);
//
var gameStart = function gameStart(response) {
...
//
var startFailure = function startFailure(response) {
  $('#game-message').html('<p>oops?</p>');
};
//
var validMove = function validMove(eventTarget, actualPlayer) {
...
//
var invalidMove = function invalidMove(response) {
...
//
var promptNew = function promptNew(player) {
  var winner = player;
  $('#game-message').html("The game is over...");
};
//
var apiFail = function apiFail(response) {
...
// }
//
var apiUpdate = function apiUpdate(response) {
  store.game = response.game;
};
//
module.exports = {
...
module.exports = {
  authHandlers: authHandlers // same as addHandlers: addHandlers
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
// imports
exports.push([module.i, "@import url(https://fonts.google.com/specimen/Do+Hyeon);", ""]);
